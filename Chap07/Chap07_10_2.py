# -*- coding: utf-8 -*-
"""
Created on Wed Apr 19 11:04:41 2023
...
def recognition():
    ...
    class_id1=np.argmax(res,axis=1)
    prob1 = np.max(res,axis=1)
    class_id2 = np.argmax(np.where(res==np.max(res,axis=1)[:,None],-np.inf,res),axis=1)
    prob2 = np.max(np.where(res==np.max(res,axis=1)[:,None],-np.inf,res),axis=1) # 추가
    for i in range(5):
        cv.putText(img, f"{class_id1[i]}({class_id2[i]})", (50+i*100,180), cv.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 1)
        cv.putText(img, f"{prob1[i]:.2f}({prob2[i]:.2f})", (50+i*100,200), cv.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 1)
